...
def list_audio_devices(p: pyaudio.PyAudio): 
    ...
    for i in range(p.get_device_count()):
        ...
        print(f"[{i}] {info.get('name','?')} — {', '.join(caps)}{tag_str} — {rate} Hz")

def parse_device_args():
    parser = argparse.ArgumentParser()
    parser.add_argument("--list-devices", action="store_true",
                        help="Liệt kê thiết bị audio rồi thoát")
    parser.add_argument("--input-device", type=int, default=None,
                        help="Index thiết bị INPUT (microphone)")
    parser.add_argument("--output-device", type=int, default=None,
                        help="Index thiết bị OUTPUT (loa)")
    return parser.parse_known_args()[0]

# Audio recording parameters
STREAMING_LIMIT = 30000  # 30 seconds in milliseconds: shortened for demo purposes, the duration limit is ~5 minutes
SAMPLE_RATE = 16000
...
class ResumableMicrophoneStream:
    ...
    def __init__(self, rate, chunk_size, input_device_index=None, output_device_index=None):
        self._rate = rate
...
def listen_print_loop(responses, stream):
    ...
    for response in responses:
        ...
        translation = translator.translate(transcript, target_language='vi', model="nmt")
